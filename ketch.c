...
/* Calculate the rmr0-k values for the kinetic parameter given */
  switch (kinParType) {
    ...
    case CL_WT_PCT:
      ...
      /* WK: Is this break here intentional ? */
      /* WK: Or should the code for the case CL_PFU be repeated here ? */
...
// Input: ntime, ketchtime, ketchtemp, alo
// Otuput: final_age, aldest_age, fmean
...
#define nbins  41
  ...
  i = ForwardModel(ktime, lengthred, kinetic_par, (double) 1.0, annealmodel, doProject,
    usedCf, kinpar, l0model, *alo, cdf, numPDFPts, pdfAxis, pdf, &oldestModelAge,
    &ftModelAge, &numPopulations, kerryTt);

  //printf("Number of populations: %d (%d)", numPopulations, i);
  ...
  *oldest_age = oldestModelAge;
  //printf("Results: age: %f, oldest age: %f, mean length: %f\n\n", ftModelAge, oldestModelAge, meanlength);
